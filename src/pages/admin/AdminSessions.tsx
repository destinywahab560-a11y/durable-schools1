...
export default function AdminSessions() {
  ...
  const handleDelete = async () => {
    if (!deleteId) return
    const { error } = await supabase.from('academic_sessions').delete().eq('id', deleteId)
    if (error) { toast.error(error.message); return }
  ...
  return (
    <div>
            ...
            <div key={s.id} className="card flex items-center justify-between">
              ...
              {s.is_current ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setDeleteId(s.id)} className="p-2 rounded-lg hover:bg-error-50 text-error-500" aria-label="Delete session">
      ...
      <ConfirmDialog
        ...
        message="Grades, attendance, and enrollments tied to this session won't be deleted, but they'll lose their term label. This cannot be undone."
        confirmLabel="Delete"
        danger
      />
    </div>
